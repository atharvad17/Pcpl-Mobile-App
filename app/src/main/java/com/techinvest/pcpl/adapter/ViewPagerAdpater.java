...
import com.techinvest.pcplrealestate.fragment.CompletedVisitFragment;
import com.techinvest.pcplrealestate.fragment.PendingvisitFragment;
...
public class ViewPagerAdpater extends FragmentStatePagerAdapter {
	...
	// the class
	public ViewPagerAdpater(FragmentManager fm, CharSequence mTitles[],
							int mNumbOfTabsumb) {
		super(fm);
	...
	@Override
	public PendingvisitFragment getItem(int position) {
	/*	if (position == 0) // if the position is 0 we are returning the First
							// tab
		{
			PendingvisitFragment pendingvisit = new PendingvisitFragment();
			return pendingvisit;
		} else // As we are having 2 tabs if the position is now 0 it must be 1
				// so we are returning second tab
		{
			CompletedVisitFragment completedVisit = new CompletedVisitFragment();
